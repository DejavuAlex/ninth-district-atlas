# 北风口·莆汾哨站（battlefield）场景生成提示词

> 用途：生成 北风口·莆汾哨站 的场景示意图。生成后命名为 `北风口·莆汾哨站.png` 放入 `assets/地区/`，再运行 `npm run optimize:images` 自动转成 `public/scenes/beifengkou.webp`。

## 画面主体
北风口·莆汾哨站：北风口一带的前沿哨站，炮声阵阵、小队往来，是反复拉锯的火线。

## 本区域独有特征（重点刻画，与其它区域区分开）
北风口·莆汾哨站，反复拉锯的前沿火线：‘北风口炮声阵阵’，前沿小队在凛冽寒风中往来出没；哨站简陋、炮火不断，是直面外敌、出生入死的一线阵地。

## 氛围
寒风呼啸、炮火不断，前沿小队在此出生入死。
