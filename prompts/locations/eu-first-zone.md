# 欧盟一区（frontier）场景生成提示词

> 用途：生成 欧盟一区 的场景示意图。生成后命名为 `欧盟一区.png` 放入 `assets/地区/`，再运行 `npm run optimize:images` 自动转成 `public/scenes/eu-first-zone.webp`。

## 画面主体
欧盟一区：外部势力代表区域，推动故事进入更大尺度的冲突。
